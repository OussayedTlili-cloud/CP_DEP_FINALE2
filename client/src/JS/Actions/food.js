...
try {
  let result= await axios.get("https://cp-dep-finale2.onrender.com/api/food/allfood")
...
try {
  ...
  let result= await axios.post("https://cp-dep-finale2.onrender.com/api/food/add-food",newFood,config)
...
export const deleteFood=(id)=> async (dispatch)=> {
    ...
    await axios.delete(`https://cp-dep-finale2.onrender.com/api/food/${id}`,config)
...
export const editFood=(id,newFood,navigate)=> async (dispatch ) => {
    ...
    await axios.put(`https://cp-dep-finale2.onrender.com/api/food/${id}`,newFood,config)
    dispatch(getFood())
    navigate(-1)
} catch( error) {
                       dispatch ({type:FAIL_FOOD,payload:error.response})
...
export const getOneFood = (id) => async (dispatch) => {
    dispatch({ type: LOAD_FOOD })
    try {
        let result = await axios.get(`https://cp-dep-finale2.onrender.com/api/food/${id}`)
